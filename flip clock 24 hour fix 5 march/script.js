...
// condition to ensure that if page is initiated after 22 hours but before 24 hours, the clock hours time will return to zero rather than showing the 4 of 24. 
if(fullHours === 23 ){
  frontBottomHours.style.cssText = ` background-image:url(${numberBottomArr[0]})`
  backTopHours.style.cssText = ` background-image:url(${numberTopArr[0]})`
}else{
...
// there seems to have been an error here.  Last midnight, all time measures changed but the bottom half of hte hour reading was a bottom zero, but the topBackHours image was a 4-top.png, which means that 4-top.png was given as topBackHours' image  when the function ran at just before 23 hours.  A 2 was added to 22 hours, and then the function should have done 24%24, giving '0', and then 0%10 gives zero also; the topBackHours should have been assigned the image 0-top.png.   My only thought is that the function performed %10 first, and then %24. That would explain why the number assigned to topBackHours was 4, because, during the function when the hour is 22, (22+2)%10 = 24%10 = 4; then the next step would have been 4%10 = 4; so if the function switched the operations, clearly 4-top.png would have been chose so that when the function ran just before midnight, at 23 hours, topFront would be assigned the previous topBackHours image, i.e. 4-top.png, so the top of the hours digit would be the upper half of the number 4.  The bottom half of the digit was correct.  

// I'm checking this function later today, but have also made a duplicate clock to check it before hand instead of having to wait until midnight.  and I'm adjusting the function in the duplicate so that at the 22hrs function, the operations %24 and %10 are separated... a variable is assigned to 22+2, then the %24 operation is performed on it.  The result is assigned a variable; then %10 is performed on the new variable, and THAT variable is used to dictate which image is accessed in the images array. This should prevent sthe switching of the order of operations (if that's what happened). After the early check, if that works, I'll set the duplicate clock to the same time as the origianl; and have them showing one above the other on the same screen.  Hopefully the duplicate will work.  If the original works then there must have been a glitch; if not then I'll use the adjusted function on the original clock. 

// PROBLEM FOUND - since the initial settings for hour give the condition that topBackHours Image  = (acDate[0] + 1)%10... this means that initially if you refresh the page when the time is in the hour 23, then acDate[0] = 23 and  (acDate[0] + 1)%10 = (23 + 1)%10 = 24%10 = 4.  So the back image will be 4-top.png and remain so because the 22hrs function is missed; that's the function that turns the top back to 0-top.png..... so the page refresh nullifies the effect of the 22 hour function, or if the clock is opened up after 22hrs but before 24hrs, the top half of hour will display a '4'.  So we have to adjust things from the initial position.  I have to wonder if this is causing problems in other areas.  I suppose the crux of the matter here is that normally, outside of the internet, you wouldn't be restarting a clock unless you had to move it or if there was a power cut.. yes that would cause issues; but usually your clock is always on...  I'll use an if condition maybe for when the initial parameters are re run after refresh. Something like ' if hrs = 23 then backTop.image =    0-top.png... Then at initiation if the hrs are 23 the image can be replaced. 

  console.log('top back updating.... ')
  if(acDateArr[0] === 22){ // if hour is 22
    let foo = acDateArr[0] + 2
    console.log(foo) // check result is '24'
    let fooAdjust = foo%24;
    console.log(fooAdjust) // check result is '0'
    let hr24Correction = fooAdjust%10
    console.log(hr24Correction)// check result is still '0'
    backTopHours.style.cssText = `background-image:url(${numberTopArr[hr24Correction]})`
console.log(backTopHours.style.backgroundImage) // check correct image has been selected - should be 0-top.png
  }else{ // if hour is not 22
    console.log(`hour: ${acDateArr[0]%10}`) // check hour units
    let fooOriginal = ((acDateArr[0] + 2)%24)%10
    console.log(fooOriginal) // check result -should be hour units + 2
    backTopHours.style.cssText = `background-image:url(${numberTopArr[fooOriginal]})`
console.log(backTopHours.style.backgroundImage) // image number should be hour unit number + 2, so if hour == 21, then image should be 3-top.png
...
// detecting when tab is clicked - if a tab other than the the one belonging to the clock is clicked, the visibility change is activated and 'clock not visible' is logged, otherwise, when the clock tab is clicked causing a visibility change the page will refresh preventing the numbers from jumbling up and misaligning. 
window.addEventListener('visibilitychange', (e) =>{
if(document.visibilityState === 'visible'){ 
    console.log('active tab is flip clock')
location.reload()

}else{
console.log('clock not visible')
